...
function App() {
  const [racks, setRacks] = useState(null); // Default to null to know it's the first load
  ...
  // Fetch racks with pagination
  const fetchRacks = async (pageNum = 1) => {
    // If it's the first page load, set racks to null to show inline spinner
    if (pageNum === 1) {
      setRacks(null);
      setHasMore(true); // Reset hasMore on a fresh load/refresh
    }
    
    setLoading(true);
    try {
      const response = await axios.get(`${API}/racks?page=${pageNum}&limit=5`);
      const newRacks = response.data;
      
      // Append new racks to existing racks
      setRacks(prevRacks => ({
        ...(prevRacks || {}), // Keep existing racks
        ...newRacks          // Add new ones
      }));
      
      // If we got back an empty object or fewer floors than we asked for, there are no more
      if (Object.keys(newRacks).length === 0 || Object.keys(newRacks).length < 5) {
        setHasMore(false);
      }
      
      setPage(pageNum); // Update the current page number
      setError("");
    } catch (err) {
  ...
  // Search racks (no change needed here, but backend is faster)
  ...
  // Get racks to display (no change)
  const displayRacks = useMemo(() => {
    ...
    return racks || {}; // Default to empty object if racks is null
  }, [searchQuery, searchResults, racks]);

  // Function to highlight search terms in text (no change)
  ...
  // Modals are now moved to separate files
    ...
    const handleDelete = async () => {
      if (window.confirm("Are you sure you want to delete this rack?")) {
        try {
          ...
          fetchRacks(1); // Refetch from page 1 after delete
        } catch (err) {
  ...
  // The full-page loading screen is removed
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-10">
          ...
          {/* --- MODIFIED SECTION --- */}
          {/* Show inline spinner only on initial load (racks is null) */}
          {racks === null && loading && (
            <div className="text-center bg-white/90 backdrop-blur-md p-12 rounded-2xl shadow-xl border border-blue-200">
          ...
          {/* Only map racks if racks is not null */}
          {racks !== null && Object.entries(displayRacks).sort(([a], [b]) => a.localeCompare(b)).map(([floor, floorRacks]) => (
        ...
        <div className="flex justify-center py-10">
          {racks !== null && hasMore && !loading && (
            <button
              onClick={() => fetchRacks(page + 1)} // Load next page
              className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-10 py-4 rounded-xl hover:from-blue-700 hover:to-blue-800 font-bold text-lg transition-all shadow-2xl flex items-center"
            >
              <span className="text-2xl mr-3">🔄</span>
              Load More Floors
            </button>
          )}
          {/* Show a small spinner when loading more pages */}
          {loading && page > 0 && (
        ...
        {/* Empty State - only show if not initial load */}
        {racks !== null && Object.keys(displayRacks).length === 0 && (
      ...
      <Suspense fallback={<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        ...
        {showAddForm && <AddRackForm setShowAddForm={setShowAddForm} fetchRacks={fetchRacks} />}
        {editingRack && <EditRackForm rack={editingRack} setEditingRack={setEditingRack} fetchRacks={fetchRacks} />}
        {viewingRack && <RackDetailModal 
          rack={viewingRack} 
